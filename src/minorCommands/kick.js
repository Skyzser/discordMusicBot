...
module.exports = async function kick(message, parameters) {
    if(!message.member.permissions.has(Permissions.FLAGS.KICK_MEMBERS)) {  // Checks if the user has the permission to kick
        message.reply('You do not have permissions to use this command!');
    } else {
        if(parameters.length === 0) {
            message.reply('Please provide a valid user tag!\nFor help, type **!help**');
        } else {
            try {
                ...
                if(member) {  // If the member exists in the guild (server)
                    if(message.author.id === member.user.id) {  // To prevent self kick
