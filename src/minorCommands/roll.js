...
module.exports = function roll(message, parameters) {
    if(parameters.length === 0) {
        message.reply('Please provide a value after the !roll command.\nFor help, type **!help**');
    } else {
        if(!parameters[0].includes('d')) {
            message.reply('Please provide the valid format after the !roll command.\nFor help, type **!help**');
        } else {
            ...
            if(Number(dice) && Number(sides)) {
                ...
                message.reply('Please provide a valid number for the amount of dice or sides (0 not valid)!\nFor help, type **!help**');
            }
        }
    }
};