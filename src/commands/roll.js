...
module.exports = function roll({ message, parameters }) {
    if(parameters.length === 0) message.reply('Please provide a value after the !roll command.\nFor help, type **!help**');
        ...
        if(!parameters[0].includes('d')) message.reply('Please provide the valid format after the !roll command.\nFor help, type **!help**');
            ...
            if(Number(dice) && Number(sides)) {
                if(dice <= 0 || sides <= 0) message.reply('You cannot have less than 1 dice or sides!');
                else if(dice > 500 || sides > 500) message.reply('Number of dice/sides is too large!');
                else {
                    var rolls = [];
                    for(var i = 0; i < dice; i++) {
                        var min = 1; 
                        var max = sides;
                        var randomRoll = Math.floor(Math.random() * (max - min + 1)) + min;
                        rolls.push(randomRoll);
                    }
                    message.reply(`Rolled ${dice}, ${sides}-sided dice:\n${rolls} `);
                }
            } else message.reply('Please provide a valid number for the amount of dice or sides (0 not valid)!\nFor help, type **!help**');
        }
    }
};