export default async function Purge({ message, parameters }) {
  if (parameters.length === 0)
    message.reply(
      "Please provide a value after the !purge command.\nFor help, type **!help**"
    );
  else {
    ...
    if (Number(total)) {
      if (total >= 2 && total <= 100) {
        ...
        message.channel.bulkDelete(total, true).then((messages) => {
          message.channel
      ...
      message.reply(
        "Please provide a valid number for the total number of messages to be deleted (0 not valid)!\nFor help, type **!help**"
      );
  }
}
